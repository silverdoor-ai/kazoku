 ...
 * deliverables and "witness" hat wearers to mark a token distribution sponsorship as completed.  After the
...
contract Seneschal is HatsModule, HatsModuleEIP712 {
   ...
   * 124     | witnessHatId    | uint256 | 32      | this                |
   * ----------------------------------------------------------------------+
   */
  ...
  // @notice returns the hatId of the witness hat
  function getWitnessHat() public pure returns (uint256) {
  ...
  function sponsor(Commitment memory commitment, bytes memory signature) public returns (bool) {
    ...
    if (commitment.extraRewardAmount > 0) {
      ...
      uint256 rewardTokenDebt = extraRewardDebt(extraRewardToken);
      uint256 updatedRewardTokenDebt = rewardTokenDebt + commitment.extraRewardAmount;
      uint256 balance = IERC20(extraRewardToken).balanceOf(address(this));
      if (updatedRewardTokenDebt > balance) {
        revert NoBalance(extraRewardToken, commitment.extraRewardAmount, balance, rewardTokenDebt);
      }
      _extraRewardDebt[extraRewardToken] = updatedRewardTokenDebt;
    }
  ...
  /**
   * @dev Allows a witness to mark a token distribution commitment as completed
   * @param commitment contains all the details of the sponsorship
   * @param signature valid signature of the commitment by the witness
   */
  function witness(Commitment calldata commitment, bytes calldata signature) public returns (bool) {
    bytes32 commitmentHash = keccak256(abi.encode(commitment));
    ...
    if (v == 0) {
      signer = address(uint160(uint256(r)));
      _authenticateHat(signer, getWitnessHat());
      _verifyContractSignature(getDigest(commitment), s, signer);
    } else {
      signer = _verifySigner(commitment, signature);
      _authenticateHat(signer, getWitnessHat());
    ...
    emit Witnessed(signer, commitmentHash);
    return true;
  }

  /**
   * @dev Allows a recipient to claim their tokens after the claim delay has passed
   * Note that the recipient's eligibility is no longer checked here; because the commitment was witnessed already
   * as completed.
   * @param commitment contains all the details of the sponsorship
  ...
  function poke(Commitment calldata commitment, string memory completionReport) public returns (bool) {
    bytes32 commitmentHash = keccak256(abi.encode(commitment));
    if (_commitments[commitmentHash] != SponsorshipStatus.Pending) {
      revert NotSponsored();
    }
    ...
    if (block.timestamp < commitment.sponsoredTime + getClaimDelay()) {
      revert PokedEarly();
    }
  ...
  function clearExpired(Commitment calldata commitment) public returns (bool) {
    ...
    SponsorshipStatus status = _commitments[commitmentHash];
    if (
      commitment.expirationTime > block.timestamp || status == SponsorshipStatus.Empty
        || status == SponsorshipStatus.Failed || status == SponsorshipStatus.Claimed
    ) {
  ...
  // pre-emptively to prevent rogue proposals from being witnessed early.
  ...
  /**
   * @dev Checks whether the signature provided is valid for the provided hash, complies with EIP-1271. A signature is
   * valid if:
  ...
  function _verifyContractSignature(bytes32 digest, bytes32 s, address signer) internal view {
    ...
    bytes4 magicValue = IERC1271(signer).isValidSignature(digest, contractSignature);
  ...
  function getDigest(Commitment memory commitment) public view returns (bytes32) {
    return _hashTypedData(
      keccak256(
        abi.encode(
          keccak256(
            "Commitment(uint256 eligibleHat,uint256 shares,uint256 loot,uint256 extraRewardAmount,uint256 timeFactor,uint256 sponsoredTime,uint256 expirationTime,string contextURL,string metadata,address recipient,address extraRewardToken)"
          ),
          ...
          keccak256(bytes(commitment.metadata)),
